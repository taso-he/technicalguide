...
![Figure 1. Data structure of the LEO I2 (Iteration 2 Standard Extract) (ADR UK, 2023)](/images/admin-data-images/LEO%20component%20data%20and%20linkages.png)

## Variables available in the LEO

They are two iterations of the LEO standard extract. The initial version LEO I1 (Iteration 1 Standard Extract) contained the NPD, ILR, HESA and HMRC and DWP. The most recent version is an extension of the LEO I1, with more years of data and additional linked datasets (Figure 1). 

The data stored in LEO links education data and labour market data, which allows tracking of students from compulsory school age into HE and the labour market. The following variables are key examples of what is available in the LEO database:[^3]

[^3]: The full list of variables available in LEO is provided in the LEO I2SE variable request form, under Section 4: Using Longitudinal Education Outcomes (LEO) datasets. (ONS, 2024a).

-   Personal characteristics,[^4] such as FSM status, age, gender, ethnicity and POLAR quintile (Participation Of Local Area).[^5]

-   Educational history, such as schools, further education (FE), Alternative Providers (APs)[^6] and HE provider attended, prior school attainment, domicile status (UK, EU and overseas), course studied, predicted grades of HE applicants, number of offers received by applicants, and qualification level achieved.
...
[^4]: These variables come from different parts of the LEO database, with some common across datasets (e.g., age, gender, ethnicity) and others unique to a single dataset. 

[^5]: LEO does not provide postcodes, substitutes such as geographic identifiers are available such as LSOA or POLAR quintiles.

[^6]: Alternative providers are HE providers that did not receive recurrent funding from the Office for Students (OfS) or other public bodies and that are not further education colleges.
...
-   Graduates working abroad after graduating
...
Unlike the NPD and HESA data, the LEO data cannot currently be matched with bespoke datasets collected by evaluators outside of the LEO system for the purposes of evaluation.[^8]

[^8]: Correct at the time of publishing in 2025
...
#| tbl-cap: Table 3. Overview of the sources of variables used in analysis undertaken by State of Life and Mime as research on whether HE addresses existing quality gaps between advantaged and disadvantaged students.
...
#| tbl-cap: Table 4. Outcomes measures for research undertaken by State of Life and Mime on whether HE addresses existing quality gaps between advantaged and disadvantaged students. *Note PAYE earning does not include any earnings from self-employment or outside the UK
