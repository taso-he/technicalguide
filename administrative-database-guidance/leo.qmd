...
The data stored in LEO links education data and labour market data, which allows tracking of students from compulsory school age into HE and the labour market. The following variables are key examples of what is available in the LEO database:[^leo-3]

[^leo-3]: The full list of variables available in LEO is provided in the LEO I2SE variable request form, under Section 4: Using Longitudinal Education Outcomes (LEO) datasets. (ONS, 2024a).

-   Personal characteristics,[^leo-4] such as FSM status, age, gender, ethnicity and POLAR quintile (Participation Of Local Area).[^leo-5]

-   Educational history, such as schools, further education (FE), Alternative Providers (APs)[^leo-6] and HE provider attended, prior school attainment, domicile status (UK, EU and overseas), course studied, predicted grades of HE applicants, number of offers received by applicants, and qualification level achieved.
...
[^leo-4]: These variables come from different parts of the LEO database, with some common across datasets (e.g., age, gender, ethnicity) and others unique to a single dataset. 

[^leo-5]: LEO does not provide postcodes, substitutes such as geographic identifiers are available such as LSOA or POLAR quintiles.

[^leo-6]: Alternative providers are HE providers that did not receive recurrent funding from the Office for Students (OfS) or other public bodies and that are not further education colleges.
